...
public class AlertViewLibraryy: UIView {
    ...
    public class func instanceFromNib() -> AlertViewLibraryy {
        if let result = UINib(nibName: "AlertView", bundle: nil).instantiate(withOwner: nil, options: nil).first as? AlertViewLibraryy {
            return result
        } else {
            print("error adding")
            let window = UIApplication.shared.keyWindow ?? UIWindow()
            let view = AlertViewLibraryy(frame: window.frame)
            return view
        }
